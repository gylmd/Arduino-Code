...
void loop() {
  ...
  for (byte relaydata = 0; relaydata <= 255; relaydata = relaydata + 32) {
    a=relaydata;
...
void SetupRelays1() {
  ...
  for (byte i = 0; i <= 7; i = i + 1) {
    ...
    MCP_Write(i, 0x113, 0b00000000);      //  set all outputs to off
  }
}
...
void WriteRelays1(byte address, byte data) {
  // Write data to relays
  // --------------------
